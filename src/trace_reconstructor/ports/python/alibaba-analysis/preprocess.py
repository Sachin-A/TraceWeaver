...
for dataset_id in range(0, num_shards + 1):
    ...
    '''
        The compression, uncompression, and deletion done in this script
        (and elsewhere) is in a streaming manner to just save space on disk
        while preprocessing.
    '''
    ...
    for i in range(len(trace_ids)):
        ...
        '''
            If a compressed file exists for output_path + trace_id, the
            trace_id has been seen before and straddles multiple shard files
        '''
    ...
    '''
        Check if past shards have traces corresponding to the same trace_id
        within a certain lookback window threshold.
    '''
    lookback = 5
