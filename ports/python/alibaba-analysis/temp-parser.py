...
def isRoot(span, trace):
    if (
        (span[3] == "0.1" and noSpanWith(trace, "0"))
        or (span[3] == "0.1.1" and noSpanWith(trace, "0.1"))
        or (span[3] == "0")
    ):
        return True
    return False
    ...
    for ep in list(span_partitions.keys()):
        ...
        for i in range(len(span_partitions[ep])):
            ...
            for j in range(pos, len(span_partitions[ep])):
                if (
                    span_partitions[ep][j][1] == span_partitions[ep][i][1]
                    and span_partitions[ep][j][3] == span_partitions[ep][i][3]
                ):
                    duplicates.add(j)
    ...
    for in_span in in_spans:
        ...
        for ep in out_span_partitions.keys():
            for k, out_span in enumerate(out_span_partitions[ep]):
                ...
                if (
                    (in_span[1] == out_span[1])
                    and (in_span[6] == out_span[4])
                    and (in_span[3] == parent_rpc_id)
                ):
                    no_answer = False
...
def AccuracyForService(pred_assignments, true_assignments, in_spans):
    ...
    for in_span in in_spans:
        print("cnt: ", cnt)
        if (
            pred_assignments[(in_span[1], in_span[3])]
            == true_assignments[(in_span[1], in_span[3])]
        ):
            cnt += 1
